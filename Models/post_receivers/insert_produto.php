...
if (!empty($codigo)) {
    ...
    $insertProdutos = \MySql::conectar()->prepare("INSERT INTO `tb_produtos` (`id`, `nome`, `valor_venda`, `codigo`, `ncm`, `CST_ICMS`, `pICMS`, `quantidade`, `pCOFINS`, `pPIS`, `CFOP`, `CST_COFINS`, `CST_PIS`, `valor_compra`, `valor_atacado`, `unid_comercial`, `unid_tributavel`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $insertProdutos->execute(array($cod,$nome,$preco_prod,$cod,$ncm,$cst_icms,$icms_prod,$quantidade_prod,$cofins,$pis,$cfop,$cst_cofins,$cst_pis,$custo_prod,$preco_atacado_prod,$unid_venda,$cst_unid_tributavel));
}
