...
$row = \MySql::conectar()->prepare("SELECT COUNT(placa_carro) as total_vendas,placa_carro FROM `tb_vendas` WHERE pre_venda != 1 AND fechada = 1 AND DATE(data) BETWEEN ? AND ?  GROUP BY placa_carro ORDER BY total_vendas DESC");
$row->execute(array($_POST['data_min'],$_POST['data_max']));
$row = $row->fetchAll();
$row_caixa_podium = \MySql::conectar()->prepare("SELECT COUNT(`tb_vendas`.id_cliente) as total_vendas, `tb_clientes`.nome FROM `tb_vendas` INNER JOIN tb_clientes ON tb_vendas.id_cliente =tb_clientes.id WHERE `tb_clientes`.nome != 'Não Informado' AND fechada = 1  AND DATE(data) BETWEEN ? AND ? GROUP BY `tb_clientes`.nome ORDER BY total_vendas DESC ");
